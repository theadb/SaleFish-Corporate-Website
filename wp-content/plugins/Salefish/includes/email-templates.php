...
function salefish_notification_email_html( array $fields, string $form_type ): string {
	...
	foreach ( $fields as $key => $val ) {
		...
		$label = ucwords( str_replace( '_', ' ', $key ) );
		if ( $key === 'linkedin_url' ) {
			$href        = esc_url( (string) $val );
			$extra_rows .= sprintf(
				'<tr><td style="color:#a1a1a1;font-size:14px;padding:8px 0;border-bottom:1px solid #2a2a2a;width:40%%;">%s</td><td style="color:#ffffff;font-size:14px;padding:8px 0;border-bottom:1px solid #2a2a2a;"><a href="%s" style="color:#a78bfa;text-decoration:none;" target="_blank" rel="noopener noreferrer">%s</a></td></tr>',
				esc_html( $label ),
				$href,
				esc_html( (string) $val )
			);
...
function salefish_send_notification( array $fields, string $form_type ): bool {
	...
	$headers = [
		...
		'From: SaleFish <admin@salefish.app>',
	];
