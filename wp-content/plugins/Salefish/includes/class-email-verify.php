...
class Salefish_Email_Verify {
	...
	public static function build_confirmation_html( string $verify_url ): string {
		...
		$body .= '<body style="margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,\'Segoe UI\',Roboto,\'Helvetica Neue\',Arial,sans-serif;background-color:#0f0f0f;color-scheme:dark;">';
		...
		if ( $logo ) {
			...
			$body .= '<img src="' . esc_attr( $logo ) . '" alt="SaleFish" style="height:40px;width:auto;margin-bottom:10px;">';
			$body .= '</td></tr>';
		}

		$body .= '<tr><td style="padding:0 40px 40px;">';
		$body .= '<h1 style="color:#ffffff;font-size:28px;font-weight:700;line-height:1.2;margin:0 0 20px;">One more step, you\'re almost in.</h1>';
		$body .= '<p style="color:#a1a1a1;font-size:16px;line-height:1.7;margin:0 0 8px;">Thanks for registering with SaleFish.</p>';
		$body .= '<p style="color:#a1a1a1;font-size:16px;line-height:1.7;margin:0 0 30px;">Click the button below to confirm your email address and complete your registration.</p>';
		...
		$body .= '</td></tr></table>';
		$body .= '<p style="color:#555555;font-size:13px;line-height:1.7;margin:0;">This link expires in 48 hours. If you didn\'t register at salefish.app, you can safely ignore this email.</p>';
		$body .= '</td></tr>';

		$body .= '<tr><td style="padding:30px 40px;border-top:1px solid #2a2a2a;text-align:center;">';
		$body .= '<p style="color:#666666;font-size:14px;margin:0 0 8px;font-weight:600;">Real Estate Inventory &amp; Sales Powered by SaleFish</p>';
		$body .= '<p style="color:#4a4a4a;font-size:13px;margin:0;"><a href="https://salefish.app/privacy-policy" style="color:#4a4a4a;text-decoration:underline;">Privacy Policy</a>&nbsp;&middot;&nbsp;<a href="https://salefish.app/terms-of-use" style="color:#4a4a4a;text-decoration:underline;">Terms of Use</a></p>';
		$body .= '</td></tr>';
	 ...
	 *
	 * Verification is idempotent: the token is marked verified rather than
	 * deleted on first click. Email security scanners (Microsoft Defender
	 * Safe Links, Mimecast, Proofpoint, etc.) routinely pre-fetch links to
	 * scan them for malware — if we deleted the token on first click, the
	 * scanner would consume it and the actual user would see "expired".
	 * Tokens are still purged after the 48-hour TTL by the daily cron.
	 */
	public static function handle_verification(): void {
		...
		// Already verified by an earlier click (likely an email-link scanner).
		// Skip duplicate registration but still send the user to thank-you.
		...
		try {
			...
			update_option( self::OPTION_PREFIX . $token, $pending, false );
		} catch ( \Throwable $e ) {
